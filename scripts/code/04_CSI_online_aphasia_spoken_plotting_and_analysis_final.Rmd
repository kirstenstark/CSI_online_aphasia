...
## As soon as we have the controls: Add factor group
...
table(df$correct)
df$VOT[df$correct==1.4] ### Was bedeutet das? VOT ist ja nicht immer NA

# Overview of incorrect responses
sum(df$correct==0, na.rm=T)

sum(df$correct == 0 & !is.na(df$error)) # einmal fehlt die Fehlerklassifizierung!! 
df[df$correct == 0 & is.na(df$error),]

## add trials where no audio file was recorded to technical errors
sum(is.na(df$VOT))
...
for(i in 1:length(unique(means_final$session))){
  for(j in 2:length(unique(means_final$PosOr))) {
    means_final$increase[means_final$session==i &
                           means_final$PosOr==j] <- 
     means_final$VOT[means_final$session==i &
                           means_final$PosOr==j] - 
      means_final$VOT[means_final$session==i &
                           means_final$PosOr==j-1]
  }
}
means_final

## Calculate overall mean increase per session (weighted)
mean(means_final$increase[means_final$session==1], na.rm=T)
means_final$PosOr_effect <- NA
means_final$PosOr_effect[means_final$PosOr==1] <- 1
for(i in 1:length(unique(means_final$session))){
  for(j in 2:length(unique(means_final$PosOr))) {
    means_final$PosOr_effect[
      means_final$session==i & means_final$PosOr==1] <-
      means_final$PosOr_effect[
      means_final$session==i & means_final$PosOr==1] + 
      means_final$increase[means_final$session==i &
                             means_final$PosOr==j]*
      (means_final$N[means_final$session==i &
                          means_final$PosOr==j]+
        means_final$N[means_final$session==i &
                          means_final$PosOr==j-1])
  }
  means_final$PosOr_effect[means_final$session==i &
                             means_final$PosOr==1] <-
    means_final$PosOr_effect[means_final$session==i &
                               means_final$PosOr==1]/
    (sum(means_final$N[means_final$session==i])+
       sum(means_final$N[means_final$session==i &
                           (means_final$PosOr==2 |
                              means_final$PosOr==3 |
                              means_final$PosOr==4)]))
...
Line graph by repetition/session (only correct trials, without fillers)
...
**Conclusion:** .
Gamma fits the data better than a normal model with uncontrolled data
...
saveRDS(m1, file = here::here("results", "tables",
                          "CSI_online_aphasia_spoken_glmm_cont.RDS"))
...
df_correct$VOTlog <- log(df_correct$VOT)
```

```{r}
library(lmerTest)
# m1_lmm <- lmer(VOTlog ~ PosOr.cont*session + 
#                (PosOr.cont*session|subject) +
#               (PosOr.cont*session|category),
#              data = df_correct, 
...
m1_lmm <- afex::lmer_alt(VOTlog ~ PosOr.cont*session +
               (PosOr.cont*session||subject) +
              (PosOr.cont*session||category),
             data = df_correct,
            control=lmerControl(optimizer = "bobyqa",
...
saveRDS(m1_lmm,  file = here::here("results", "tables", "CSI_online_aphasia_spoken_control_lmm_VOT.RDS"))
...
Only analyses errors that were made by the participants (exclude technical errors)

```{r}
df %>% mutate(errors = case_when(correct == 0 ~ 1,
                                 correct != 0 ~ 0)) -> df
df_errors <- df[df$error != 99 | is.na(df$error),]
```

### Descriptives

```{r descriptives_errors}
(means_final_errors<- df_errors %>% 
   filter(category != "Filler") %>% 
   Rmisc::summarySEwithin(.,"errors",idvar = "subject",
                          withinvars = c("session", "PosOr"),
                          na.rm = T))
(means_final_cat_errors <- df_errors %>% 
   filter(category != "Filler") %>% 
   Rmisc::summarySEwithin(.,"errors",idvar = "category",
                          withinvars = c("session", "PosOr"),
                          na.rm = T))
(means_final_wo_session_errors <- df_errors %>% 
   filter(category != "Filler") %>% 
   Rmisc::summarySEwithin(.,"errors",idvar = "subject",
                          withinvars = c("PosOr"), na.rm = T))
# Export as word file
...
```{r}
means_final_errors$increase <- NA
for(i in 1:length(unique(means_final_errors$session))){
  for(j in 2:length(unique(means_final_errors$PosOr))) {
    means_final_errors$increase[means_final_errors$session==i &
                           means_final_errors$PosOr==j] <- 
     means_final_errors$errors[means_final_errors$session==i &
                           means_final_errors$PosOr==j] - 
      means_final_errors$errors[means_final_errors$session==i &
                           means_final_errors$PosOr==j-1]
  }
}
#means_final_errors

## Calculate overall mean increase per session (weighted)
mean(means_final_errors$increase[means_final_errors$session==1], na.rm=T)
...
for(i in 1:length(unique(means_final_errors$session))){
  for(j in 2:length(unique(means_final_errors$PosOr))) {
    means_final_errors$PosOr_effect[
      means_final_errors$session==i & means_final_errors$PosOr==1] <-
      means_final_errors$PosOr_effect[
      means_final_errors$session==i & means_final_errors$PosOr==1] + 
      means_final_errors$increase[means_final_errors$session==i &
                             means_final_errors$PosOr==j]*
      (means_final_errors$N[means_final_errors$session==i &
                          means_final_errors$PosOr==j]+
        means_final_errors$N[means_final_errors$session==i &
                          means_final_errors$PosOr==j-1])
  }
  means_final_errors$PosOr_effect[means_final_errors$session==i &
                             means_final_errors$PosOr==1] <-
    means_final_errors$PosOr_effect[means_final_errors$session==i &
                               means_final_errors$PosOr==1]/
    (sum(means_final_errors$N[means_final_errors$session==i])+
       sum(means_final_errors$N[means_final_errors$session==i &
...
(plot_errors <- means_final_wo_session_errors %>% 
    ...
    geom_point(size = 2)+
    geom_errorbar(aes(ymin=errors*100-se*100,
                      ymax=errors*100+se*100),
                  width =.1) +
    stat_summary(fun=mean,  geom="line", size = 0.5, group=1, linetype="dashed")+
    apatheme+
    #scale_y_continuous(limits = c(1120, 1340), breaks =seq(1120,1340, by = 20)) +
...
filename <- "CSI_online_spoken_plot_errors_by_repetition.pdf"
...
```{r plot_RTs_all}
(plot_RTs_all <- ggplot(data=df_errors, 
                        aes(x=trial, y=errors, group=session,
                            color=session)) +
  stat_summary(fun=mean,  geom="point", size = 2)+
  stat_summary(fun=mean,  geom="line", size = 1) +
  apatheme+
...
filename <- "CSI_online_aphasia__spoken_plot_errors_across_experiment.pdf"
ggsave(plot_RTs_all, filename = 
         here::here("results", "figures", filename),
       width = 18, height = 13, units = "cm", 
       dpi = 300, device = cairo_pdf)
...
# save model output
saveRDS(m1_error, file = here::here("results", "tables", "CSI_online_aphasia_spoken_glmm_errors.RDS"))
