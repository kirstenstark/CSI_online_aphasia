---
title: '05 CSI online spoken: Typing - Plotting and analysis'
...
# load data
df <- read.csv2(here::here("data", input))
...
table(df$subject, df$testing)

# how many non-respomses
table(is.na(df$RT), df$subject, df$testing)
```
...
# } else if(classification_type == "jaro") {
#   df$answercode <- df$answer_auto_jaro
#   df$correct <- df$correct_auto_jaro
# } else if(classification_type == "lv") {
#   df$answercode <- df$answer_auto_lv
#   df$correct <- df$correct_auto_lv
# } else {
...
# # table(df$PosOr[df$category != "Filler" & df$correct == 1], 
# #       df$correct[df$category != "Filler" & df$correct == 1])
```
...
(means_final_cat<- df %>% 
   filter(category != "Filler") %>% 
   Rmisc::summarySEwithin(.,"RT",idvar = "category",
                          withinvars = "PosOr", na.rm = T))
(means_final_testing<- df %>% 
   filter(category != "Filler") %>% 
   Rmisc::summarySEwithin(.,"RT",idvar = "subject",
                          withinvars = c("testing","PosOr"), na.rm = T))
# Export as word file
library(flextable)
huxt_word <- huxtable::huxtable(means_final_testing)
huxt_word <- huxtable::set_number_format(huxt_word, round(2))
huxtable::quick_docx(huxt_word, 
                     file = here::here("results", "tables",
                                       "CSI_online_subject_RT_by_repetition.docx"), 
                                       open = FALSE)
...
filename <- "CSI_online_aphasia_spoken_plot_rt.pdf"
...
```{r plot_rt}
(plot_rt_repetition <- means_final_testing %>% 
    ggplot(., aes(x=PosOr, y=RT, group=testing, color = testing)) +
    geom_point(size = 2)+
    stat_summary(aes(linetype=testing),fun=mean,  geom="line", size = 0.5) +
    scale_linetype_manual(values=c("dashed", "dotted", "solid"))+
    scale_color_manual(values=c("#0072B2", "#E69F00", "#000000"))+
    geom_errorbar(aes(ymin=RT-se, ymax=RT+se, group = testing), width =.1) +
    apatheme+
    #scale_y_continuous(limits = c(1120, 1340), breaks =seq(1120,1340, by = 20)) +
                       #breaks = c(1100, 1150, 1200, 1250, 1300, 1350)) + 
    labs(x="Ordinal Position ",y ="RT (ms)", colour="Repetition", linetype="Repetition") + #+
  # annotate(geom="text", x=1.5, y=1330, label="n = 30", 
  #         color="black", size = 8))
    theme(
    axis.title.y = element_text(margin = margin(0,10,0,0)),
...
filename <- "CSI_online_aphasia_typing_boxplot.pdf"
...
filename <- "CSI_online_aphasia_typing_plot_rts_across_experiment.pdf"
...
filename <- "CSI_online_aphasia_typing_plot_rts_across_experiment_correct_experimental_trials.pdf"
...
df_valid <- df %>% filter(category != "Filler") %>% 
  filter(correct == 1) %>% 
  droplevels()
df$testing <- as.factor(df$testing)
```
...
```{r GLMM_cont}
m1 <- glmer(RT ~ PosOr.cont*testing + 
               (PosOr.cont|subject) +
              (PosOr.cont|category),
             data = df_valid, 
...
tab_model(m1,transform = NULL,
          ...
          #string.pred = "",
          string.stat = "t-Value",
          file = here::here("results", "tables", "CSI_online_aphasia_typing_glmm_cont.html"))
```
