---
title: "Power Analysis for CSI online typing with patients with aphasia - Interaction effect: Session x Ordinal position (repetition estimates)"
...
Subset the data to the three repetitions of the first session.

```{r}
df_Ord <- df %>% filter(wh == 1) %>% droplevels()
```
...
df_Ord %>% mutate(OrdPos_num = case_when(OrdPos == "OP1" ~1,
        ...
        mutate(repet=as.factor(repet))-> df_Ord
```
...
boxcox(df_Ord$RT ~ df_Ord$OrdPos*df_Ord$repet)
...
contrasts(df_Ord$repet) <- contr.sdif(3)
...
#### 30 ms interaction effect, 20 VP

```{r}
gm <- mean(df_Ord$RT)
eff_30ms <- (1000/(gm+(30/2))) - (1000/(gm-(30/2)))
fixef(lmm24_20)["OrdPos.c:repet2-1"]<- eff_30ms
fixef(lmm24_20)["OrdPos.c:repet3-2"]<- eff_30ms
```

*Model B1: Linear model with interaction effect "Ordinal position x Session" of 35 ms* 

```{r PowerSim_Interaction30_20_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction30_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 

```{r}
PowerLMM1_Interaction30_20
```

```{r PowerSim_Interaction30_20_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction30_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
#### 30 ms interaction effect, 25 VP

```{r}
gm <- mean(df_Ord$RT)
eff_30ms <- (1000/(gm+(30/2))) - (1000/(gm-(30/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_30ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_30ms
```

*Model B1: Linear model with interaction effect "Ordinal position x Session" of 30 ms* 

```{r PowerSim_Interaction30_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction30 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 

```{r}
PowerLMM1_Interaction30
```


```{r PowerSim_Interaction30_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction30 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
fixef(lmm24_20)["OrdPos.c:repet2-1"]<- eff_35ms
fixef(lmm24_20)["OrdPos.c:repet3-2"]<- eff_35ms
```

*Model B1: Linear model with interaction effect "Ordinal position x Session" of 35 ms* 

```{r PowerSim_Interaction35_20_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction35_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction35_20_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction35_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
gm <- mean(df_Ord$RT)
eff_35ms <- (1000/(gm+(35/2))) - (1000/(gm-(35/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_35ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_35ms
```

*Model B1: Linear model with interaction effect "Ordinal position x Session" of 35 ms* 

```{r PowerSim_Interaction35_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction35 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction35_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction35 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
fixef(lmm24_20)["OrdPos.c:repet2-1"]<- eff_40ms
fixef(lmm24_20)["OrdPos.c:repet3-2"]<- eff_40ms
fixef(lmm20_Stark)["OrdPos.c:repet2-1"]<- eff_40ms
fixef(lmm20_Stark)["OrdPos.c:repet3-2"]<- eff_40ms
```

*Model B2: Linear model with interaction effect "Ordinal position x Session" of 40 ms* 

```{r PowerSim_Interaction40_20_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction40_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction40_20_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction40_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction40_20_Stark <- powerSim(lmm20_Stark, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 

```{r}
PowerLMM1_Interaction40_20_Stark
```


```{r PowerSim_Interaction40_20_Stark_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction40_20_Stark <- powerSim(lmm20_Stark, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
gm <- mean(df_Ord$RT)
eff_40ms <- (1000/(gm+(40/2))) - (1000/(gm-(40/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_40ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_40ms
```

*Model B2: Linear model with interaction effect "Ordinal position x Session" of 40 ms* 

```{r PowerSim_Interaction40_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction40 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction40_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction40 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction45_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction45_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
gm <- mean(df_Ord$RT)
eff_45ms <- (1000/(gm+(45/2))) - (1000/(gm-(45/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_45ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_45ms
```

*Model B3: Linear model with interaction effect "Ordinal position x Session" of 45 ms* 

```{r PowerSim_Interaction45_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction45 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction45_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction45 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction50_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction50_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
gm <- mean(df_Ord$RT)
eff_50ms <- (1000/(gm+(50/2))) - (1000/(gm-(50/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_50ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_50ms
```

*Model B4: Linear model with interaction effect "Ordinal position x Session" of 50 ms* 

```{r PowerSim_Interaction50_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction50 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction50_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction50 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction55_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
PowerLMM1_Interaction55_20 <- powerSim(lmm24_20, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
gm <- mean(df_Ord$RT)
eff_55ms <- (1000/(gm+(55/2))) - (1000/(gm-(55/2)))
fixef(lmm24_25)["OrdPos.c:repet2-1"]<- eff_55ms
fixef(lmm24_25)["OrdPos.c:repet3-2"]<- eff_55ms
```

*Model B5: Linear model with interaction effect "Ordinal position x Session" of 55 ms* 

```{r PowerSim_Interaction55_2-1, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction55 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet2-1","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
...
```{r PowerSim_Interaction55_3-2, message=FALSE, warning=FALSE, cache=TRUE, results="hide"}
PowerLMM1_Interaction55 <- powerSim(lmm24_25, test=fixed("OrdPos.c:repet3-2","t"), nsim = n_it) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
``` 
